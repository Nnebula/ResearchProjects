...
//void WLC(int n, double time,double Xt, double *ex,double *force)
//{
//    double Xp=0.0;
//    Xp=50e-9*time+Xp;
//    ex[n]=Xt-Xp;
//    force[n]=(kb*temp/50e-9)*((1/(2*(1-ex[n]/340e-9)))*(1/(2*(1-ex[n]/340e-9)))+(ex[n]/340e-9)-0.25);
//}


void Langevin_position_Fokker_WLC(double *time,double *position)
{
    time[0]=0.0;
    ofstream outPS;
    outPS.open("/Users/Naanaa/Desktop/PS_Data/WLC.csv");

    position[0]=0.0;
    double extension[L],force[L];
    int n=0;
    double t;
    t=time[n];
    double Xp=0.0;
    ...
    do
    {
        time[n+1]=time[n]+dts;
        Xp=50e-9*dts+Xp;
        extension[n]=position[n]-Xp;
        force[n]=(kb*temp/50e-9)*((1/(4*(1-extension[n]/340e-9)))*(1/(4*(1-extension[n]/340e-9)))+(extension[n]/340e-9)-0.25);
        position[n+1]=position[n]+D1*dts*position[n]+force[n]*1e-12*dts+(sqrt(D2*dts)*rand_normal(0.0, 2.0));
        outPS<<n<<","<<time[n]<<","<<extension[n]<<","<<force[n]<<endl;
        n+=1;
        cout<<position[n]<<endl;
    }while(force[n]<2e-12);
     outPS.close();
...
int main()
{
    ...
    ofstream outPS;
    outPS.open("/Users/Naanaa/Documents/research/Trap_WLC_simulation/Powerspectrum/PS100kHz.txt");
